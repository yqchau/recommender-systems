...
class BPRMF(nn.Module):
    ...
    def forward(self, user, item):
        ...
        return torch.sum(torch.mul(user_emb, item_emb), 1)
    
    def fit(self, num_epoch=101):
        optimizer = torch.optim.Adam(self.parameters(), lr=self.lr, weight_decay=self.reg)
        criterion = nn.LogSigmoid()
        for epoch in range(num_epoch):
            data = self.dataset.get_train()
            train_data = torch.tensor(data)
            user, item_pos, item_neg = train_data[:,0], train_data[:,1], train_data[:,2]
            ...
            for batch in range(0, max_idx, self.batch_size):
                ...
                pred_pos = self.forward(user[idx], item_pos[idx])
                pred_neg = self.forward(user[idx], item_neg[idx])
                #print(labels[idx])
                #print(output)
                loss = -torch.sum(criterion(pred_pos - pred_neg),0)/pred_pos.shape[0]
                loss.backward()
                optimizer.step()
            if epoch % 10 == 0:    
